...
import NotificationService from "../../../services/users/notification-service"
    ...
    return (
        <div className={"col-12"}>
            <div className={"container"}>
                <div className={"row justify-content-center border-bottom py-2"} onClick={() => handleGoTo(notification)}>
                    <div className="col-12">
                            <div style={{whiteSpace:"initial"}}>{notification.message}</div>
                    </div>
                    <div className={"col-12"}>
                        <div className="text-secondary mt-2 font-italic">{moment(notification.createdAt).fromNow()}</div>
                    </div>
                </div>
            </div>
        </div>
...
export default function NavBarNotifications({
    ...
    const viewAll = async () => {
        await router.push(getDashboardLink(user) + "/account")
    }
        ...
        <div className="notifications cursor-pointer dropdown">
            ...
            <div className="dropdown-menu dropdown-menu-right shadow mt-md-0 mr-md-3 mt-5">
                ...
                <div className="d-flex justify-content-between">
                    <button className="dropdown-item mt-2 pt-2 text-center text-white font-weight-bolder" type="button" onClick={viewAll}>View all
                    </button>
                    {notifications.length > 0 &&
                    <button className="dropdown-item mt-2 pt-2 text-center font-weight-bolder" type="button" onClick={markAllAsRead}>Mark
                        all as read</button>}
                </div>
            </div>
        </div>
