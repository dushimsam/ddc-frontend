...
import {replaceCharacter, updateJavaScriptObject} from '../../utils/functions'
            ...
            <div className={"modal-dialog modal-dialog-centered"}>
                <div className={"modal-content " }>
                    <div className={"modal-body "}>
                        <LoginToConfirmComponent SYSTEM_USER={system_user} continueAction={updateUser}
...
const BasicInformation = () => {
    ...
    const updateRecords = async () => {
        ...
        if (!updateState) {
            ...
            if (updateState === "BOTH" || updateState === "DETAILS") {
                if (userDetails.username !== values.username || userDetails.email !== values.email) {
    ...
    const removeAnImage = () => {
        filesService.delete(replaceCharacter(userDetails.profile, '%2F', '/'), "PROFILE", userDetails._id)
            .then((res) => {
                notifySuccess("File removed successfully");
                refresh();
                setUserDetails({...userDetails, imageUrl: "https://apis.koreaautop.com/api/files/load/", profile: ""})
            }).catch(e => console.log(e))
    }
    return (
        <div>
            ...
            <div className="row mt-3">
                    ...
                    </div>
                    {
                        user.category.name === "EMPLOYEE" ?
                            <div className="form-group">
                                ...
                                <input className="form-control" id="national_id" value={nationalId}/>
                ...
                <div className="d-block d-md-block col-sm-6 col-md-2 mt-2 text-center">
                    ...
                    {userDetails?.profile !== "" ?
                        <div className="text-center mt-3">
                            <a href="#" className="text-danger" onClick={removeAnImage}>Remove</a>
                        </div> : <></>
                    }
                </div>
            </div>
            ...
            {<ConfirmModal system_user={userDetails?.category?.name} updateUser={updateUser}/>}
        </div>
    )
}
