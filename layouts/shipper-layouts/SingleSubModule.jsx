...
import ShipperDashboard from "../dashboardsV2/ShipperDashboard";
                        ...
                        {showFilter ? (
                            <div className={(isOrder) ? 'col-8': "col-5"}>
                                <div className={'mb-3'}>
                                    ...
                                    {filters.map((filter) => (
                                        <FilterButton name={filter.name} val={filter.val} vals={(filter.vals) ? filter.vals: null} setFilter={setFilter}/>
                                    ))}
                                </div>
                            </div>
                        ): null}
...
const FilterButton = ({name, val, vals, setFilter}) => {
    return (
        <React.Fragment>
              <button style={styles.button}  onClick={() => {(vals) ? setFilter(vals) : setFilter(val)}}>
                  {name}
              </button>
        </React.Fragment>
    )
