...
export const convertFromRdaToUsd = (francs, v) =>
  numFormatter(Math.ceil(0.00098 * francs * 100) / 100, v);
export const convertFromRdaToSkw = (francs, v) =>
  numFormatter(Math.ceil(1.18 * francs * 100) / 100, v);
export const convertFromRdaToUae = (francs, v) =>
  numFormatter(Math.ceil(0.0036 * francs * 100) / 100, v);
export const convertFromRdaToEuro = (francs, v) =>
  numFormatter(Math.ceil(0.00085 * francs * 100) / 100, v);
export const defaultCurrency = (francs, v) => numFormatter(francs, v);
export const globalConverterMapping = (currency, francs, v) =>
  currency === dollarCurrency
    ? convertFromRdaToUsd(francs, v)
    : currency === euroCurrency
    ? convertFromRdaToEuro(francs, v)
    : currency === koreanCurrency
    ? convertFromRdaToSkw(francs, v)
    : currency === dubaiCurrency
    ? convertFromRdaToUae(francs, v)
    : defaultCurrency(francs, v);

export const currencyMapping = (currency, francs) =>
  currency === dollarCurrency
    ? "$" + convertFromRdaToUsd(francs)
    : currency === euroCurrency
    ? "€" + convertFromRdaToEuro(francs)
    : currency === koreanCurrency
...
export const customCurrencyMapping = (currency, user, francs) => {
  return currencyMapping(
    user?.category?.name === system_users.CUSTOMER ? currency : rwandanCurrency,
    francs
  );
};
