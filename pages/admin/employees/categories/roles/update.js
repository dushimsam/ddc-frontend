...
import EmployeeRolesService from "../../../../../services/employees/employee-roles"
...
const Content = ({item, getInitialData}) => {
    ...
    useEffect(() => {
        EmployeeRolesService.get(item._id)
            .then((res) => {
                setValues({
                    role: res.data.role,
    ...
    const Update = () => {
        EmployeeRolesService.update(item._id, values)
            .then((res) => {
                alertSuccess(setAlert, "Employee Role Updated")
                getInitialData();
                ...
                <div className="col-10 align-self-center">
                    <FormLayout
                        Content={<FormContent setIsFormValid={setIsFormValid} setValues={setValues} values={values}
                                              status={"update"}/>} alert={alert} title={"Update Employee Role"}
...
const Update = ({item, getInitialData}) => {
    return <UpdateFormLayout Content={<Content item={item} getInitialData={getInitialData}/>}
                             title={"CHANGE EMPLOYEE ROLE DETAILS"}/>
}
