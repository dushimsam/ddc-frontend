...
import {alertFailer, notifyError, notifyInfo, notifySuccess} from "../../../../utils/alerts";
...
export default function PayOrder() {
        ...
        if (files.length > 0) {
            try {
                if (shippingInfo.amount > 1) {
                    ...
                    let payment_res = await PaymentService.createOrderPayment(paymentValues);
                    for (let u = 0; u < files.length; u++) {
                        const formData = new FormData();
                        formData.append("receipt", files[u]);
                        await PaymentService.addOrderReceipts(payment_res.data._id, formData);
                    }

                    notifySuccess("Successfully sent the payment documents.");
                    notifyInfo("We are going to prompt you about the delivery of your order as soon as possible.");
                    setLoading(false)
                    window.setTimeout(() => {
    ...
    useEffect(() => {
        if (order) {
            ShipmentService.getPortPricingByPortCountryVehicle(order.booking.delivery_port._id, order.booking.car_on_market.supplied_car.country_origin._id, order.booking.car_on_market.supplied_car.vehicle_type._id)
                .then((res) => {
                    setShippingInfo({amount: res.data.delivery_price, days: res.data.transfer_time});
                    setAmountToPay(res.data.delivery_price + order.booking.car_on_market.selling_price)
                }).catch(e => {
                console.log(e.response ? e.response.data.message : e.message || "Error occurred. Try again latter.")
            });
        }
    }, [order])
        ...
        <RouteProtector only={[system_users.CUSTOMER]}>
            {
                ...
                <span className="text-dark font-weight-bold mr-2">
                    Order Payment
                                ...
                                <div>
                                    <div
                                        className={"badge  font-weight-bold " + (order.status === "PAYING" ? "badge-warning" : order.status === "PAID" ? "badge-success" : "badge-success")}>
                                        {order.status}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="card-body px-md-5 pb-1">
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-6 text-right">
                                        {currencyMapping(currency, order.booking.car_on_market.selling_price)}
                                    </div>
                                    ...
                                    {discountAmount !== 0 && (
                                        <>
                                            ...
                                            <div
                                                className="text-right">{currencyMapping(currency, discountAmount)}</div>
                                        </>
                                    )}
                                    <div className="col-6">
                        ...
                        <div className="row pt-4 mb-4">
                            <div className="col text-left">
                                <Link
                                    href={gotoPathDirect("/customer/book-now/review", order.booking._id)}
                                    passHref
                                >
